...
TUNING_GRID = {
    ...
    "max_name_features": [2000, 3000, 5000],
    "ngram_max": [1, 2],
    "tfidf_min_df": [1, 2],
    "C": [0.5, 1.0, 2.0],
}
    ...
    cuisine_frequency = (
        pd.DataFrame(
            [
                ...
                for cuisine, count in all_cuisine_counts.items()
            ]
        )
...
def build_model(
    ...
    return Pipeline(
        steps=[
            ("preprocessor", build_preprocessor(max_name_features, ngram_max, tfidf_min_df)),
...
def clean_feature_name(feature_name: str) -> str:
    if feature_name.startswith("name__"):
        return "Restaurant Name: " + feature_name.replace("name__", "", 1)
...
def extract_feature_importance(model: Pipeline, cuisine_names: np.ndarray, top_n_per_cuisine: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    preprocessor = model.named_steps["preprocessor"]
    classifier = model.named_steps["classifier"]

    feature_names = np.array([clean_feature_name(name) for name in preprocessor.get_feature_names_out()])
    coefficient_matrix = np.vstack([estimator.coef_.ravel() for estimator in classifier.estimators_])
    mean_abs_coefficients = np.mean(np.abs(coefficient_matrix), axis=0)

    overall_importance = (
        pd.DataFrame(
            {
                "Feature": feature_names,
                "Mean Absolute Coefficient": mean_abs_coefficients,
            }
        )
    ...
    cuisine_top_features = []
    for cuisine_index, cuisine in enumerate(cuisine_names):
        coefficients = coefficient_matrix[cuisine_index]
        top_indexes = np.argsort(coefficients)[::-1][:top_n_per_cuisine]
        for rank, feature_index in enumerate(top_indexes, start=1):
            cuisine_top_features.append(
                {
                    "Cuisine": cuisine,
                    "Rank": rank,
    ...
    return overall_importance, pd.DataFrame(cuisine_top_features)
...
def save_results(
    ...
    cuisine_top_features: pd.DataFrame,
    ...
    cuisine_top_features.to_csv(results_dir / "top_features_by_cuisine.csv", index=False)
...
def save_plots(
    ...
    fig.update_layout(
        template=template,
        title="Cuisine Classification Metrics by Dataset Split",
    ...
    fig = go.Figure()
    for index, metric in enumerate(["Micro F1", "Macro F1", "Sample F1", "Exact Match Accuracy", "Hamming Loss"]):
        fig.add_trace(
            go.Scatter(
                ...
                "buttons": [
                    {
                        ...
                        "method": "update",
                        "args": [
                            {"visible": [name == metric for name in ["Micro F1", "Macro F1", "Sample F1", "Exact Match Accuracy", "Hamming Loss"]]},
                            {"yaxis": {"title": metric}},
                        ],
                    }
                    for metric in ["Micro F1", "Macro F1", "Sample F1", "Exact Match Accuracy", "Hamming Loss"]
                ],
                "direction": "down",
                "x": 1.12,
                "y": 1.15,
    ...
    if cv_summary is not None and not cv_summary.empty:
        ...
        fig.update_layout(
            template=template,
            title="Cross-Validation Metrics by Hyperparameter Candidate",
        ...
        params = ["min_cuisine_samples", "max_name_features", "ngram_max", "tfidf_min_df", "C"]
        fig = go.Figure()
        for index, param in enumerate(params):
            fig.add_trace(
                go.Scatter(
                    x=cv_summary[param],
                    y=cv_summary["Mean Micro F1"],
                    mode="markers",
                    name=param,
                    marker={"size": 12},
                    text=cv_summary["Parameters"],
        ...
        fig.update_layout(
            template=template,
            title="Mean CV Micro F1 vs Hyperparameter Values",
            xaxis_title=params[0],
            yaxis_title="Mean Micro F1",
            updatemenus=[
                {
                    "buttons": [
                        {
        ...
        fig.write_html(results_dir / "cv_micro_f1_vs_hyperparameters_toggle.html", include_plotlyjs=True)

    if cv_folds is not None and not cv_folds.empty:
        fold_metric_names = ["Micro F1", "Macro F1", "Sample F1", "Hamming Loss", "Exact Match Accuracy"]
        fig = go.Figure()
        trace_metric_names = []
    ...
    metric_names = ["F1-score", "Precision", "Recall", "Support"]
    fig = go.Figure()
    for index, metric in enumerate(metric_names):
    ...
    top_feature_counts = [20, 40, min(75, len(feature_importance))]
    top_feature_counts = list(dict.fromkeys(count for count in top_feature_counts if count > 0))
    fig = go.Figure()
    for index, top_n in enumerate(top_feature_counts):
        top_features = feature_importance.head(top_n).sort_values("Mean Absolute Coefficient")
        fig.add_trace(
            go.Bar(
                x=top_features["Mean Absolute Coefficient"],
                y=top_features["Feature"],
                orientation="h",
                name=f"Top {top_n}",
                text=top_features["Mean Absolute Coefficient"].round(4),
                textposition="auto",
                visible=index == 0,
            )
        )
    fig.update_layout(
        ...
        xaxis_title="Mean Absolute Coefficient",
        yaxis_title="Feature",
        height=800,
        updatemenus=[
            {
                "buttons": [
                    {
                        ...
                        "method": "update",
                        "args": [
                            {"visible": [count == top_n for count in top_feature_counts]},
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train TF-IDF + OneVsRest Logistic Regression for multi-label restaurant cuisine classification."
    )
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--model-output",
        default="cuisine_tfidf_logistic_model.pkl",
        help="Output path for the trained cuisine classification pipeline",
    )
    parser.add_argument(
        "--label-binarizer-output",
        default="cuisine_label_binarizer.pkl",
        help="Output path for the fitted MultiLabelBinarizer",
    )
    parser.add_argument(
        "--results-dir",
        default="cuisine_model_tfidf_logistic_results",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Minimum number of restaurants required for a cuisine label to be modeled",
    )
    ...
    parser.add_argument("--threshold", type=float, default=None, help="Prediction threshold. If omitted, tune on validation data.")
    parser.add_argument("--max-name-features", type=int, default=3000, help="Maximum TF-IDF features from Restaurant Name")
    parser.add_argument("--ngram-max", type=int, default=2, help="Maximum TF-IDF n-gram length for Restaurant Name")
    parser.add_argument("--tfidf-min-df", type=int, default=2, help="Minimum document frequency for Restaurant Name TF-IDF terms")
    parser.add_argument("--regularization-c", type=float, default=1.0, help="Logistic Regression C value")
    parser.add_argument("--top-n-features", type=int, default=15, help="Top positive features saved for each cuisine")
    parser.add_argument("--n-jobs", type=int, default=1, help="Parallel jobs for OneVsRestClassifier")
    parser.add_argument("--random-state", type=int, default=42, help="Random seed")
    parser.add_argument("--cv-folds", type=int, default=3, help="Number of K-fold cross-validation folds used during tuning")
    parser.add_argument("--tuning-trials", type=int, default=8, help="Number of hyperparameter candidates to test")
    parser.add_argument("--skip-tuning", action="store_true", help="Train with provided/default parameters without cross-validation tuning")
    args = parser.parse_args()

    report("Cuisine Classification: TF-IDF + OneVsRest Logistic Regression")
    ...
    report(f"Minimum cuisine samples: {args.min_cuisine_samples}")
    ...
    model = build_model(
        max_name_features=best_params["max_name_features"],
        ngram_max=best_params["ngram_max"],
        tfidf_min_df=best_params["tfidf_min_df"],
        regularization_c=best_params["C"],
        n_jobs=args.n_jobs,
    )
    ...
    feature_importance, cuisine_top_features = extract_feature_importance(
        model,
        mlb.classes_,
        top_n_per_cuisine=args.top_n_features,
    )

    report("\nTop Overall Features")
    report(feature_importance.head(25).to_string(index=False))
    ...
    save_results(
        ...
        cuisine_top_features=cuisine_top_features,
