...
def add_map_legend(folium_map, title: str, items: list[tuple[str, str]], note: str | None = None) -> None:
    from branca.element import Element

    rows = "".join(
        f"""
        <div style="display:flex;align-items:center;gap:8px;margin:4px 0;">
            <span style="width:13px;height:13px;border-radius:50%;background:{color};display:inline-block;border:1px solid #111827;"></span>
            <span>{label}</span>
        </div>
        """
        for label, color in items
    )
    note_html = f"<div style='font-size:11px;color:#4b5563;margin-top:6px;'>{note}</div>" if note else ""
    legend_html = f"""
    <div style="
        position: fixed;
        bottom: 24px;
...
def add_heatmap_legend(folium_map) -> None:
    ...
    <div style="
        ...
        max-width: 240px;
    ">
        <div style="font-weight:700;font-size:13px;margin-bottom:8px;">Restaurant Density</div>
        <div style="height:12px;width:190px;background:linear-gradient(to right,#2563eb,#22c55e,#facc15,#ef4444);border:1px solid #111827;"></div>
        <div style="display:flex;justify-content:space-between;width:190px;margin-top:4px;">
            <span>Low</span><span>High</span>
        </div>
        <div style="font-size:11px;color:#4b5563;margin-top:6px;">Warmer areas contain more restaurants.</div>
    </div>
    """
    folium_map.get_root().html.add_child(Element(legend_html))
...
def create_folium_maps(df: pd.DataFrame, output_dir: Path, max_map_points: int) -> None:
    ...
    for row in sampled.to_dict(orient="records"):
        ...
        marker_color = rating_color(float(row["Aggregate rating"]))
        folium.CircleMarker(
    ...
    add_map_legend(
        ...
        items=[
            ("Excellent: 4.5+", "#166534"),
            ("Very good: 4.0-4.4", "#22c55e"),
            ("Average: 3.0-3.9", "#f59e0b"),
            ("Low: below 3.0", "#ef4444"),
            ("Not rated / zero", "#6b7280"),
        ],
        note="Marker color is based on Aggregate rating.",
    )
